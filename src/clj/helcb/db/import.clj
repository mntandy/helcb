...
(defn prepare-journey-row-for-import [m]
  (let [result (dissoc (merge (zipmap (columns/db-keys :journeys) (repeat ""))
...
(defn convert-to-int [m] 
  (-> m
      (update :distance bigint)
      (update :duration bigint)))
    
(defn import-or-print! [m]
  (let [renamed-m (rename-keys m (columns/label->key :journeys))]
    (if (and (check-station-id renamed-m) (integer-str-distance-and-duration? renamed-m))
      (db/insert-row! (prepare-journey-row-for-import (convert-to-int renamed-m)))
      (println renamed-m))))

(defn journeys-from-csv [params] 
  (csv/import-from-uri (:uri params) (:sep params)
                         import-or-print!
                         (columns/csv-labels :journeys)))
